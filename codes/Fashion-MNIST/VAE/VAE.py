...
class VAE(object):
    ...
    def decoder(self, z, is_training=True, reuse=False):
        ...
        with tf.variable_scope("decoder", reuse=reuse):
            ...
            net = tf.nn.relu(
                bn(deconv2d(net, [batch_size, 14, 14, 64], 4, 4, 2, 2, name='de_dc3'), is_training=is_training,
                   scope='de_bn3'))
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.num_batches):
                ...
                if np.mod(counter, 50) == 0:
                    save_path = self.save(self.checkpoint_dir, counter)
                ...
                if np.mod(counter, 300) == 0:
                    ...
                    if self.z_dim == 2:
                        y_PMLR = np.array([self.sess.run(self.y_out, feed_dict={self.z_in: self.PMLR.z[i].reshape([1,2])}) for i in range(400)])
                        y_PMLR_img = y_PMLR.reshape(self.PMLR.n_tot_imgs, 28, 28)
                        self.PMLR.save_images(y_PMLR_img, name="/PMLR_epoch_%d" % (counter) + ".jpg")
    ...
    def visualize_results(self, counter):
        tot_num_samples = min(self.sample_num, self.batch_size)
        ...
        save_images(samples[:image_frame_dim * image_frame_dim, :, :, :], [image_frame_dim, image_frame_dim],
                    check_folder(
                        self.result_dir + '/' + self.model_dir) + '/' + self.model_name + '_counter%d' % counter + '_test_all_classes.png')
        ...
        if self.z_dim == 2:
            ...
            save_scattered_image(z_tot, id_tot, -4, 4, name=check_folder(
                self.result_dir + '/' + self.model_dir) + '/' + self.model_name + '_counter%d' % counter + '_learned_manifold.png')

            
    def generate_images(self, z0, iter_num):
        self.saver.restore(self.sess, "checkpoint/VAE_fashion-mnist_128_2/VAE/VAE.model-%d" %iter_num)
        samples = self.sess.run(self.fake_images, feed_dict={self.z: z0})
        return samples        
