...
if dim == 5:
  schedule = np.array(np.round(grid/sum(grid)*463),dtype=int)
  stepsize = 0.01
elif dim == 10:
  schedule = np.array(np.round(grid/sum(grid)*1915),dtype=int)
  stepsize = 0.01
elif dim == 20:
  schedule = np.array(np.round(grid/sum(grid)*15445),dtype=int)
  stepsize = 0.001
...
with tf.Session() as sess:
    ...
    for i in range(n):
        print(i)
        (sol,nstep) = gan.seq_adamGD(init[i], true[i], stepsize, schedule, inter)
